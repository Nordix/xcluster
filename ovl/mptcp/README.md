...
ip mptcp endpoint add 192.168.6.221 dev eth2 signal
staprun mptcpapp.ko                     # (will hang)
ctraffic -address 192.168.1.2:5003 -monitor -rate 10 --nconn 10 --timeout 5m
# Drop and trace on vm-201 and vm-202
```


## Kernel

If the kernel is built locally, kernel test scripts and programs will
be included.
...
cd $__kobj
make INSTALL_HDR_PATH=$(readlink -f .)/sys headers_install

# Kernel test scripts;
# (they trash for each-other so execute on different VMs)
cd /bin
./pm_netlink.sh
./mptcp_connect.sh
./mptcp_join.sh
...
#Doc and info;
eval $($XCLUSTER env | grep -E '__kver|KERNELDIR')
cd $KERNELDIR/$__kver
grep -ir mptcp Documentation/  # (zip...)
find . -name '*mptcp*'
ls ./net/mptcp
ls ./tools/testing/selftests/net/mptcp
less ./Documentation/networking/tls.rst # TCP_ULP (Upper Layer Protocol
```
